...
namespace OsuSharp.Analyzer
{
    public sealed class RecentScoreAnalyzer : Analyzer<long, List<UserRecent>>
    {
        ...
        public override async Task<List<UserRecent>> UpdateEntityAsync(long key)
        {
            ...
            var scores = await Api.GetUserRecentByUseridAsync(key, _entities[key][0].GameMode, _limit);
